...
impl Renderer {
    ...
    unsafe fn create_pixel_shader(device: &ID3D11Device) -> Result<ID3D11PixelShader> {
        ...
        let shader = {
            let mut shader = None;
            device.CreatePixelShader(PIXEL_SHADER, None, Some(&mut shader))?;
            shader.unwrap()
        };
        Ok(shader)
    }
...
impl StateBackup {
    unsafe fn backup(context: Option<ID3D11DeviceContext>) -> Self {
        ...
        result.rasterizer_state = ctx.RSGetState().map_or(None, |state| Some(state));
        ctx.OMGetBlendState(
        ...
        result.input_layout = ctx.IAGetInputLayout().map_or(None, |layout| Some(layout));
    ...
    pub fn restore(mut self) {
        unsafe {
            ...
            if let Some(state) = self.blend_state {
                ctx.OMSetBlendState(&state, Some(&self.blend_factor), 0xFFFFFFFF);
            } else {
                ctx.OMSetBlendState(None, Some(&self.blend_factor), 0xFFFFFFFF);
            }
            if let Some(state) = self.depth_stencil_state {
                ctx.OMSetDepthStencilState(&state, self.stencil_ref);
            } else {
                ctx.OMSetDepthStencilState(None, self.stencil_ref);
            }
            ctx.PSSetShaderResources(0, Some(&self.shader_resource));
            ctx.PSSetSamplers(0, Some(&self.sampler));
            if let Some(shader) = self.ps_shader {
                ctx.PSSetShader(&shader, Some(&inst));
            } else {
                ctx.PSSetShader(None, Some(&inst));
            }
            if let Some(shader) = self.vs_shader {
                ctx.VSSetShader(&shader, Some(&vinst));
            } else {
                ctx.VSSetShader(None, Some(&vinst));
            }
            ctx.VSSetConstantBuffers(0, Some(&self.constant_buffer));
            if let Some(shader) = self.gs_shader {
                ctx.GSSetShader(&shader, Some(&[]));
            } else {
                ctx.GSSetShader(None, Some(&[]));
            }
            ctx.IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
